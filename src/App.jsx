...
// //       const a = document.createElement('a');
// //       a.href = url;
// //       a.download = `${documentTitle || 'plan'}-preview-v${currentVersion}.pdf`; // <-- PDF extension
// //       document.body.appendChild(a);
// //       a.click();
// //       a.remove();
// //       window.URL.revokeObjectURL(url);
...
// //         const combinedHtml = htmlSections
// //           .map(s => extractBodyContent(s.content || s.html || ''))
// //           .join('<hr/>');
...
// //               plugins: [grapesjsPresetWebpage, grapesjsTable],
// //               blockManager: {
// //                 appendTo: '.myblocks',
// //               },
// //               styleManager: {
...
// //             // Add custom commands **after** editorRef.current is set
// //             editorRef.current.Commands.add('show-layers', {
// //               run(editor) {
// //                 editor.Panels.getButton('layers', 'show-layers').set('active', true);
// //                 editor.LayerManager.render();
// //               },
// //             });
// //             editorRef.current.Commands.add('show-style', {
// //               run(editor) {
...
//   const handleUsePresignedUrl = () => {
//     const presignedUrl =
//       'https://helios-document-vault.s3.amazonaws.com/uploads/plan_templates/disaster_recovery_plan/c441d69b-0d85-4d88-90f5-1d33d72c4196.docx?AWSAccessKeyId=AKIA565JV2OVDQD5SJ3I&Expires=1751652130&Signature=MP4eOJp4TipbnXbHdHRIqI1o4Oo%3D';

//     setFileUrl(presignedUrl);
//     setFileName('disaster_recovery_plan.docx');
//   };
...
//         title: fileName,
//         url: fileUrl, // ✅ Direct S3 pre-signed URL
//       },
//       documentType: 'word',
//       editorConfig: {
...
//       <h2>📄 Load Pre-signed S3 DOCX in OnlyOffice</h2>
//       <button onClick={handleUsePresignedUrl}>Load Document</button>

//       {fileUrl && (
//         <>
//           <h4 style={{ marginTop: '20px' }}>{fileName}</h4>
//           <div
...
// Basic toolbar component with formatting buttons
  ...
  return (
    <div style={{ marginBottom: '8px' }}>
      ...
      <button
        ...
        style={{ fontWeight: editor.isActive('heading', { level: 1 }) ? 'bold' : 'normal', marginRight: 8 }}
        type="button"
      >
        H1
      </button>
      <button
        ...
        style={{ fontWeight: editor.isActive('heading', { level: 2 }) ? 'bold' : 'normal', marginRight: 8 }}
        type="button"
      >
        H2
      </button>
      ...
      <button
        ...
        style={{ fontWeight: editor.isActive('orderedList') ? 'bold' : 'normal', marginRight: 8 }}
        type="button"
      >
        Ordered List
      </button>
...
// Editor component for each section with toolbar
...
const App = () => {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/update`, {
      ...
      body: JSON.stringify({ card_id: '6863d13ac883130a35a2014b' }),
    })
      .then((res) => res.json())
      .then((data) => {
        if (data.success && data.data.html_content) {
          setSections(data.data.html_content);
          setEditedSections(data.data.html_content.map((section) => section.html));
        }
      });
  }, []);

  const handleSectionUpdate = (index, updatedHtml) => {
    const updated = [...editedSections];
    updated[index] = updatedHtml;
    setEditedSections(updated);
  };

  const saveAllSections = () => {
    const payload = sections.map((section, i) => ({
      title: section.title,
      html: editedSections[i],
    }));

    fetch('/api/save-updated-html', {
      method: 'POST',
  ...
  return (
    <div style={{ maxWidth: 800, margin: 'auto', padding: 20 }}>
      {sections.map((section, i) => (
        <div key={i}>
          <h2>{section.title}</h2>
          <SectionEditor content={section.html} onUpdate={(html) => handleSectionUpdate(i, html)} />
        </div>
      ))}
      <button
